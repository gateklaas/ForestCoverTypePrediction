...
Determine the best C parameter (where crossv error is at minimum)
"""
...
clf = SVC(kernel="rbf", gamma=0.55, cache_size=1000)
response = lc.crossv_c(clf, train_X, train_y, crossv_X, crossv_y)
lc.plot_crossv_c(*response, title="SVM with gaussian kernel")

clf = SVC(kernel="linear", cache_size=1000)
response = lc.crossv_c(clf, train_X, train_y, crossv_X, crossv_y)
lc.plot_crossv_c(*response, title="SVM with linear kernel")
